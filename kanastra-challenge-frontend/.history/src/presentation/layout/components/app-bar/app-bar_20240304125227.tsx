...
const CheckActiveUrl = (page: string): boolean => {
  ...
  console.log("page: ", page, location.pathname.indexOf(page));

  if (location.pathname == "/" && page == "dashboard") return true;
  return location.pathname.indexOf(page) != -1;
};

console.log(CheckActiveUrl("boletos"));

function AppBarComponent() {
  const navigation = [
    { name: "Dashboard", href: "/", current: CheckActiveUrl("/") },
                    ...
                    {navigation.map((item) => (
                      <a
                        ...
                        className={classNames(
                          item.current
                            ? "bg-[#14392a] text-white"
                            : "text-[#14392a] hover:bg-[#14392a] hover:text-white",
